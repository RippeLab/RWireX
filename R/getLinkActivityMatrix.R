...
getLinkActivityMatrix <- function (ArchRProj, CoAccessibility, useMatrix="PeakMatrix", peakSubset=NULL, rangeForSubset=NULL, binarize=TRUE, correlationCoefficient=FALSE, logFile = createLogFile("getLinkActivityMatrix"), verbose=TRUE){
  ...
  ArchR:::.validInput(input = rangeForSubset, name = "rangeForSubset", valid = c("GRanges", "GRangesList", " IntegerRanges", "null"))
